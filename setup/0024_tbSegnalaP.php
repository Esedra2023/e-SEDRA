...
if($_POST['resume'] === '1') {  //SQL SERVER
    if(stripos($_SESSION['ini']['dbms'], 'SQL Server') === 0){
        $resume = 'if exists(select TABLE_NAME from INFORMATION_SCHEMA.TABLES where TABLE_NAME = \'segnalaCommP\') DROP TABLE segnalaCommP;';
    }
    else if($_SESSION['ini']['dbms'] == 'My SQL'){
        $resume = 'DROP TABLE IF EXISTS segnalaCommP;';
    }
    $sql = $resume . $sql;
}
...
echo "Table segnalaCommP creata...";
exit();
