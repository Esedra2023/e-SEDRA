...
CREATE TABLE news
(
	idNw INT NOT NULL $autoinc PRIMARY KEY,
	utente INT NOT NULL,
...
if($_POST['resume'] === '1') {  //SQL SERVER
    if(stripos($_SESSION['ini']['dbms'], 'SQL Server') === 0){
        $resume = 'if exists(select TABLE_NAME from INFORMATION_SCHEMA.TABLES where TABLE_NAME = \'news\') DROP TABLE news;';
    }
    else if($_SESSION['ini']['dbms'] == 'My SQL'){
        $resume = 'DROP TABLE IF EXISTS news;';
    }
    $sql = $resume . $sql;
}
