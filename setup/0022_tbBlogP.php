...
(
    ...
    FOREIGN KEY(proposta) REFERENCES proposte(idPr) ON DELETE NO ACTION,
    FOREIGN KEY (idMaster) REFERENCES blogP(idBl) ON DELETE NO ACTION
);
SQL;
if($_POST['resume'] === '1') {  //SQL SERVER
    if(stripos($_SESSION['ini']['dbms'], 'SQL Server') === 0){
        $resume = 'if exists(select TABLE_NAME from INFORMATION_SCHEMA.TABLES where TABLE_NAME = \'blogP\') DROP TABLE blogP;';
    }
    else if($_SESSION['ini']['dbms'] == 'My SQL'){
        $resume = 'DROP TABLE IF EXISTS blogP;';
    }
    $sql = $resume . $sql;
}
...
echo "Table blogProposte creata...";
exit();
